...
def ask_plot(x, y, annotations, acq_time):
	...
	plt.xlabel('Time (s)')
	plt.xlim(acq_time[0],acq_time[1])
	plt.show()
...
def main():
	...
	#if status_text != 0:
	#	print status_text
	avt, Fs, status_text = get_avt(rsa)
	...
	#the slop
	acq_time = np.zeros(2)
	acq_time[0] = float(rsa.ask('display:avtime:x:scale:offset?'))
	acq_time[1] = float(rsa.ask('display:avtime:x:scale:full?'))
